...
if __name__ == "__main__":
    ...
    def play_game_mcts_alphabeta(game, time_limit = 60):
        ...
        while True:
            ...
            if game.current_player == 1:
                best_move, iteration_count_alg1 = mcts3(game, is_to_move=game.current_player, time_limit=120)
                if not best_move:
                    winner = 2
                    break
                ...
                best_move, iteration_count_alg2 = iterative_deepening(game, max_depth=5, alpha_beta_player=game.current_player, time_limit=30)
                if not best_move:
                    winner = 1
                    break
                game.make_move(*best_move)
    ...
    def play_game_alphabeta_mcts(game, time_limit = 60):
        ...
        while True:
            ...
            if game.current_player == 1:
                best_move, iteration_count_alg1 = iterative_deepening(game, max_depth=5, alpha_beta_player=game.current_player, time_limit=30)
                print(best_move)
                if not best_move:
                    winner = 2
                    break
                ...
                best_move, iteration_count_alg2 = mcts3(game, is_to_move=game.current_player, time_limit=120)
                if not best_move:
                    winner = 1
                    break
                game.make_move(*best_move)
    ...
    def save_game_stats(game_count, move_count, move_stats_alg1, move_stats_alg2, iteration_count_alg1_list, iteration_count_alg2_list, winner, algo_1, algo_2, time_limit, filename='game_stats.json'):
        filename_final = os.path.join(r"C:\Users\Timmy\Desktop\Test 5.7 Aysmmetrischer Test MCTS AB 30 120", filename)
        try:
