...
def pvs(game, depth, alpha, beta, maximizing_player, best_move_prev=None):
    ...
    if maximizing_player:
        ...
        for move in moves:
            ...
            game.make_move(*move)
            if first_move:
                eval, _ = pvs(game, depth-1, alpha, beta, False) # Full window search
                first_move = False
            else:
                eval, _ = pvs(game, depth-1, alpha, alpha+1, False) # Null window search
                if eval > alpha:
                    eval, _ = pvs(game, depth-1, alpha, beta, False) # Full window search if necessary
        ...
        for move in moves:
            ...
            game.make_move(*move)
            if first_move:
                eval, _ = pvs(game, depth-1, alpha, beta, True) # Full window search
                first_move = False
            else:
                eval, _ = pvs(game, depth-1, beta-1, beta, True) # Null window search
                if eval < beta:
                    eval, _ = pvs(game, depth-1, alpha, beta, True) # Full window search if necessary
...
def iterative_deepening_pvs(game, max_depth, alpha_beta_player, time_limit):
    ...
    for depth in range(1, max_depth + 1):
        if timeout:
            break
        eval, move = pvs(game, depth, float('-inf'), float('inf'), maximizing_player, best_move_prev)
        if not timeout:
            best_move = move
            best_move_prev = move
